...
const ToolbarButton = ({ active, onMouseDown, children }) => {
  return (
    <button
      className={`p-2 m-1 rounded ${active ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
      onMouseDown={event => {
...
const TextEditor = () => {
  ...
  const renderElement = useCallback(props => {
    switch (props.element.type) {
      ...
      case 'heading-two':
        return <h2 className="text-xl font-bold my-2" {...props.attributes}>{props.children}</h2>;
      case 'blockquote':
        return <blockquote className="border-l-4 pl-3 italic my-2" {...props.attributes}>{props.children}</blockquote>;
      default:
        return <p className="my-1" {...props.attributes}>{props.children}</p>;
    }
  }, []);
