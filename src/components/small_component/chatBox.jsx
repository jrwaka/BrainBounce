  ...
  return (
    <div className="flex h-[600px] w-full rounded-xl shadow-md bg-blue-100">
      ...
      <div className="w-1/3 border-r p-4 relative bg-gray-100">
        <button
          onClick={hidingChatBox}
          className="bg-gray-600 p-[4px] text-white rounded absolute top-0 left-0"
        >
          <IoMdClose size={25} />
        </button>
        <h2 className="text-lg font-semibold mb-4 mt-5 whitespace-nowrap">
          Available Teachers
        </h2>
        <ul>
        ...
        {selectedTeacher ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {messages.map((msg, index) => (
                <div
                  key={index}
                  className={`mb-2 ${
                    msg.sender === "parent" ? "text-right" : "text-left"
                  }`}
                >
                  <span
            ...
            <div className="py-2 border-t flex bg-white bg">
              ...
              <button
                onClick={sendMessage}
                className="ml-2 mr-4 bg-white cursor-pointer text-blue-500 rounded"
              >
                <FiSend className=" md:size-7" />
              </button>
            </div>
          </>
        ) : (
